...
struct Node{
	...
	Node(){ depth = 0;  child = 0; index = 0;	theta = 0;	label = 1;	err = 0;	weight = 0; };
};
...
struct DTreeParams{
	...
	DTreeParams(){ maxDepth = MAX_DTREE_DEPTH; isBinary = 1; saveBinary = 0; };
};
...
private:	
	...
	void trainrec2(Data* tdata, vector<int>* indexes, T3* oi, int treeindex);
	//io stuff
	ofstream fout;
	ifstream fin;
