...
public:
	...
	T predictCascadeFixed(T* sample, float cascade_th = 0);	/** Predict from the sample array (fastest) using rejection thresholds. Evaluates the decision function until it is larger than the rejection threshold for each stage. \param cascade_th Amount to decrease the provided constant rejection threshold. Negative values delay rejection.*/
	
	void test(int test_nr);
